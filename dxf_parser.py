...
def find_line_number(doc):
    """
    Находит и извлекает номер линии из углового штампа.

    Примечание: Эта функция — самая сложная часть, так как структура
    углового штампа может сильно отличаться. Тебе нужно будет
    адаптировать ее под свои чертежи.

    Возможные подходы:
    1. Поиск текста в определенных координатах.
    2. Поиск текста рядом с определенным заголовком (например, "Номер линии").
    3. Поиск атрибутов блока, если штамп — это блок с атрибутами.
    """
    # Пример: ищем текст, начинающийся с "Линия-"
    msp = doc.modelspace()
...
def extract_specification(doc):
    """
    Извлекает спецификацию материалов из чертежа.

    Примечание: Как и с номером линии, этот код нужно будет адаптировать.
    Спецификация может быть представлена в виде таблицы из линий и текста,
    или как блок с атрибутами.

    Этот пример ищет текст, который выглядит как строки спецификации.
    """
    msp = doc.modelspace()
    raw_specification = []
    pipes_specification = []
    # Пример: ищем строки, где есть "Труба" и "шт" или "м"
    # Логика здесь будет сильно зависеть от структуры твоей спецификации
...
def parse_dxf_file(file_path):
    """
    Главная функция для парсинга одного DXF файла.
    """
